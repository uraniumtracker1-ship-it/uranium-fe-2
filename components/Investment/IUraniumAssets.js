...
import { uraniumAssetsData } from "@/public/static-data/uraniumAssetsData";
...
const IUraniumAssets = () => {
  ...
  const uniqueOptions = useMemo(() => {
    return {
      companyType: ["All", ...new Set(uraniumAssetsData.map(item => item["Company Type"]).filter(Boolean))],
      country: ["All", ...new Set(uraniumAssetsData.map(item => item["Domiciled"]).filter(Boolean))],
      stage: ["All", ...new Set(uraniumAssetsData.map(item => item["Company Type"]).filter(Boolean))],
      mineLocation: ["All", ...new Set(uraniumAssetsData.map(item => item["Mine Location Country"]).filter(Boolean))],
      primaryResource: ["All", ...new Set(uraniumAssetsData.map(item => item["Primary Assets"]).filter(Boolean))],
    };
  }, []);

  // Filter and sort data
  const filteredAndSortedData = useMemo(() => {
    let filtered = uraniumAssetsData.filter((asset) => {
      const matchesSearch = 
